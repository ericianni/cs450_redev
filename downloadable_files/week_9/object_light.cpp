	              ...
	              const float time) const
{
    ...
    if (mvLoc >= 0)
        ...
        std::cerr << "Failed to get uMV location" << std::endl;
...
void Object::draw(const glm::mat4& modelFromStack,
                  const glm::mat4& view,
                  const glm::mat4& projection,
                  const Light& light,
	              const float time) const
{
    ...
    // Combine Transforms from Stack with modelMatrix
    glm::mat4 finalModelMatrix = modelFromStack * modelMatrix;

    // Compute combined matrix MV
